...
def seed_database():
    ...
    try:
        ...
        classes_data = [
            ...
            ("ECE-3rd Year A", ece_dept.id),
            ("ECE-3rd Year B", ece_dept.id)
        ]
        ...
        # 8. Seed Non-Teaching Administrative Staff (NON_TEACHING role)
        non_teaching_data = [
        ...
        for name, email, role in non_teaching_data:
            user = db.query(User).filter(User.email == email).first()
            if not user:
                user = User(
                    email=email,
